...
from utils import get_network, get_teacher_network, get_training_dataloader, get_test_dataloader, get_training_dataloader_CIFAR, get_test_dataloader_CIFAR
import ipdb
...
def train(epoch, args):
    ...
    for batch_index, (images, labels) in enumerate(CIFAR_training_loader):
        ...
        with autocast():
            ...
            time_windows = 4
            correct += preds.eq(labels).sum()
            kd_loss = F.kl_div(F.log_softmax(outputs / time_windows, dim=1),
                        F.softmax(teacher_outputs.detach() / time_windows, dim=1),
                        reduction='batchmean') * time_windows * time_windows
            cls_loss = loss_function(outputs, labels)
            loss = 0.3 * kd_loss + cls_loss
            running_loss += loss.item()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-teacher_net', type=str, required=True, help='teacher_net type')
    ...
    teacher_net = get_teacher_network(args)
    teacher_net.cuda()
    ...
    if args.dataset == 'cifar10':
        ...
        teacher_path = "checkpoint_ds_kd/cifar10_kd_resnet34.pth"  # change to your ckpt
    elif args.dataset == 'cifar100':
        source_checkpoint = 'checkpoint_ds_kd/cifar100_pycls34.pyth'
        # Transform model weight to complete the initialization
        downsample_load_resnet(source_checkpoint, net)
        teacher_path = "checkpoint_ds_kd/cifar100_kd_resnet34.pth"  # change to your ckpt
    map_location = {'cuda:%d' % 0: 'cuda:%d' % args.local_rank}
    # Load teacher model
    teacher_checkpoint = torch.load(teacher_path, map_location=map_location)
    teacher_net.load_state_dict(teacher_checkpoint)

    teacher_net = nn.SyncBatchNorm.convert_sync_batchnorm(teacher_net)
    teacher_net = torch.nn.parallel.DistributedDataParallel(teacher_net, device_ids=[args.local_rank])
    ...
    checkpoint_path = os.path.join('checkpoint_T=4', args.dataset, args.net, 'ds+kd', str(args.b), str(args.lr), settings.TIME_NOW)
    result_path = os.path.join("result", 'checkpoint_T=4', args.dataset, 'ds+kd', str(args.b), str(args.lr), settings.TIME_NOW)
    if args.local_rank == 0:
