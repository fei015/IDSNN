...
import numpy as np
import down_sample.core.logging as logging
import torch
from down_sample.core.config import cfg
from down_sample.core.timer import Timer
...
class TestMeter(object):
    ...
    def get_top1_err(self):
        return self.num_top1_mis / self.num_samples
...
class KDTrainMeter(object):
    ...
    def __init__(self, epoch_iters):
        ...
        self.kd_loss = ScalarMeter(cfg.LOG_PERIOD)
        self.loss_total = 0.0
        self.cls_loss_total = 0.0
        self.kd_loss_total = 0.0
        self.lr = None
        # Current minibatch errors (smoothed over a window)
        self.mb_top1_err = ScalarMeter(cfg.LOG_PERIOD)
        self.mb_top5_err = ScalarMeter(cfg.LOG_PERIOD)
    ...
    def update_stats(self, top1_err, top5_err, loss, cls_loss, kd_loss, lr,
                     mb_size):
        # Current minibatch stats
        self.mb_top1_err.add_value(top1_err)
        self.mb_top5_err.add_value(top5_err)
        ...
        self.kd_loss.add_value(kd_loss)
        self.lr = lr
        # Aggregate stats
        self.num_top1_mis += top1_err * mb_size
        self.num_top5_mis += top5_err * mb_size
        ...
        self.kd_loss_total += kd_loss * mb_size
        self.num_samples += mb_size
    ...
    def get_iter_stats(self, cur_epoch, cur_iter):
        ...
        stats = {
            ...
            "kd_loss": self.kd_loss.get_win_median(),
            "lr": self.lr,
            "mem": int(np.ceil(mem_usage)),
        }
        return stats
    ...
    def get_epoch_stats(self, cur_epoch):
        ...
        top5_err = self.num_top5_mis / self.num_samples
        avg_loss = self.loss_total / self.num_samples
        avg_cls_loss = self.cls_loss_total / self.num_samples
        avg_kd_loss = self.kd_loss_total / self.num_samples
        stats = {
            "epoch": "{}/{}".format(cur_epoch + 1, cfg.OPTIM.MAX_EPOCH),
            "time_avg": self.iter_timer.average_time,
            "time_epoch": self.iter_timer.average_time * self.epoch_iters,
    ...
    def get_epoch_avg_loss(self):
        return (self.loss_total / self.num_samples,
                self.cls_loss_total / self.num_samples,
                self.kd_loss_total / self.num_samples)
