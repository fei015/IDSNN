...
class ResNet_origin_18(nn.Module):
    def __init__(self, block, num_block, num_classes_cifar100=100):
        super().__init__()
        k = 1
        self.in_channels = 64 * k
        self.conv1 = nn.Sequential(
            # Snn_Conv2d(3, 64*k, kernel_size=7, padding=3, bias=False, stride=2),# cifar kernel=3, stride=1
            Snn_Conv2d(3, 64*k, kernel_size=3, padding=3, bias=False, stride=1),
            batch_norm_2d(64*k),
            )
        # self.pool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)# cifar去掉

        self.mem_update = mem_update()
        # self.conv2_x = self._make_layer(block, 64*k, num_block[0], 2)# cifar stride=1
        ...
        # self.fc = nn.Linear(512 * block.expansion*k, num_classes)# cifar 100
        self.fc = nn.Linear(512 * block.expansion*k, num_classes_cifar100)
...
def resnet18(dataset):
    if dataset == 'cifar100':
        ...
        numberclass = 10
    return ResNet_origin_18(BasicBlock_18, [2, 2, 2, 2], numberclass)
...
def resnet34(dataset):
    if dataset == 'cifar100':
        ...
        numberclass = 10
    return ResNet_origin_18(BasicBlock_18, [3, 4, 6, 3], numberclass)
